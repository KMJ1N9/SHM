...
校园二手交易小程序（广州应用科技学院肇庆校区 C2C 二手交易平台）。

**技术栈（前端）：** uni-app (Vue 3 Composition API) + Pinia + SCSS → 微信小程序

**技术栈（后端 — 双后端并存，共享同一 API 契约）：**
- **Node.js 后端**（`server/`，生产可用）：Express 5 层 + MySQL + mysql2（无 ORM）+ JWT 双 Token + vitest + supertest + winston + PM2
- **Java 分布式后端**（`server-java/`，Phase 0~11 完成）：Spring Boot 3.2 + MyBatis + Spring Security + Spring Cloud Gateway + Nacos + Sentinel + Redis + Flyway + JUnit 5 + Mockito

**共享基础设施：** 腾讯云 IM + COS / GitHub Actions CI（Node.js + Java 双流水线）

**当前阶段：** Node.js 后端 13 轮迭代全部完成 ✅ | Java 后端 Phase 0~11 完成 ✅，Phase 12 待开始。双后端细节见 [docs/双后端切换说明.md](docs/双后端切换说明.md)。
...
npx vitest run                      # 全量测试 207 用例
npx vitest run --coverage           # 覆盖率
npx vitest                          # 监听模式
```

### Java 分布式后端（server-java/）
...
# 启动顺序：core → admin → im-connector → gateway（或各终端分别启动）
...
mvn -pl gateway spring-boot:run          # http://localhost:8080（统一入口）

# 测试
mvn test                                 # 全量测试 32 文件
mvn -pl common test                      # 仅 common 模块
mvn -pl core-service test               # 仅 core-service
mvn -pl core-service test -Dtest="AuthServiceTest"  # 单个测试类
```
...
### Java 分布式架构（server-java/）

```
gateway (:8080) ──→ core-service (:8081)    ← 商品/订单/用户/评价/举报/搜索
  │                 admin-service (:8082)   ← 管理后台/敏感词/数据看板/审计日志
  │                 im-connector (:8083)    ← IM UserSig 生成/COS STS 临时密钥
  │
  └── Nacos (:8848) — 服务发现 + 配置中心
      Sentinel — 流控 + 熔断 + Feign 降级
      Redis (:6379) — Cache-Aside 缓存 + Token 黑名单 + 分布式锁

common/ — 共享库（实体类/DTO/枚举/ErrorCode/ResponseBuilder/JwtUtil/SensitiveWordFilter）
```

**各模块职责：**

- **gateway/** — Spring Cloud Gateway 统一入口：路由转发 + JWT 校验（JwtAuthGatewayFilter）+ CORS + Sentinel 流控。白名单路径与 Node.js 一致
- **core-service/** — 核心业务：Controller → Service → Repository → Mapper（MyBatis XML）。JWT 鉴权（OncePerRequestFilter + tokenVersion + 封禁检查）+ 订单 FOR UPDATE 悲观锁 + Redis 缓存
- **admin-service/** — 管理后台：用户管理/商品下架/数据看板/审计日志/敏感词库。通过 Feign 调用 core-service（X-Internal-Token 共享密钥鉴权）
- **im-connector/** — IM + COS 连接器：UserSig 生成（HMAC-SHA256）+ COS STS 临时密钥。内部 API 共享密钥鉴权
- **common/** — 5 服务共享：14 实体类 + DTO/枚举 + ErrorCode（30 枚举，与 Node.js 一一对应）+ BusinessException + ResponseBuilder + PageResult + JwtUtil + SensitiveWordFilter

**与 Node.js 后端的契约一致性：**
- 相同端点路径（43 个 API）、相同响应格式 `{ code, message, data }`、相同错误码体系（30 个 ErrorCode）、相同 JWT 鉴权行为
- 前端改一行 `BASE_URL` 即可切换后端。API 对比验证方式见 [docs/双后端切换说明.md](docs/双后端切换说明.md)

**关键设计模式：**
- `FOR UPDATE` 悲观锁 — confirmOrder/cancelOrder 防并发状态错乱（与 Node.js transaction() 等价）
- `@Cacheable` + `@CacheEvict` — Redis Cache-Aside 模式（商品列表/用户信息）
- Feign + `@SentinelResource` — 微服务间调用 + Fallback 降级
- Flyway 幂等建表 — `CREATE TABLE IF NOT EXISTS`（兼容 MySQL 8.0+）
- `application.yml` 所有敏感值通过 `${ENV_VAR}` 注入，禁止硬编码
...
- 用户请求 → HTTPS → Gateway (:8080) → core-service/admin-service/im-connector → MySQL/Redis
- 聊天消息 → WebSocket → 腾讯云 IM（直连，不经后端）
- 图片上传 → 服务端发 STS 临时密钥 → 前端直传 COS（不经后端）
- 系统消息 → 后端 → IM REST API → 推送到用户
- 微服务内部调用 → Feign + X-Internal-Token → Sentinel 流控熔断
...
| 0006 | MyBatis + Mapper XML | 与 Node.js 原生 SQL 风格一致，比 JPA/Hibernate 更透明 |
| 0007 | Spring Cloud Gateway + Nacos | 微服务统一入口 + 服务发现，课程知识点覆盖 |
| 0008 | Flyway 幂等建表 | 版本化迁移，`CREATE TABLE IF NOT EXISTS` 兼容重复执行 |
...
| `miniprogram/src/api/index.js` | API 入口 + BASE_URL（切换后端改这一行） | 后端切换时 |
| `server/.env.example` | Node.js 环境变量模板（25+ 变量） | 配置环境时 |
| `server-java/.env` | Java 环境变量集中文件（不入库，受 gitignore 保护） | 配置 Java 后端环境时 |
| `server-java/pom.xml` | 父 POM（5 子模块 + 依赖版本管理） | 修改依赖/新增模块时 |
| `server-java/common/` | 共享库（ErrorCode/ResponseBuilder/JwtUtil/SensitiveWordFilter） | 任何 Java 模块开发前 |
| `server-java/IMPLEMENTATION-PLAN.md` | Java 后端 12 Phase 详细计划（269 子步骤） | Java 后端开发必读 |
| `server-java/DISTRIBUTED-CAPABILITIES-PLAN.md` | Phase 13~17 分布式核心能力补全计划（Seata/RocketMQ/Zipkin/Redisson/Sentinel） | 分布式能力开发 |
...
| `docs/前端架构文档.md` | 前端页面树/组件依赖/Store/Api 层设计 | 前端开发必读 |
| `docs/PRD-校园二手交易小程序.md` | 产品需求文档 | 理解功能边界 |
| `docs/双后端切换说明.md` | Node.js ⇄ Java 双后端架构关系/切换方式/对比验证 | 后端开发 |
| `docs/测试计划.md` | 134 条测试用例 | 写测试时参考 |
| `docs/运维手册.md` | 部署/备份/监控/故障处理 | 部署时 |
| `docs/编码迭代计划.md` | Node.js 13 轮迭代详细计划 | 理解开发历程 |
...
**本项目技术栈是 uni-app (Vue 3) + SCSS + Node.js/Express + Spring Boot/MyBatis + MySQL，非 React + Tailwind。任何 skill 调用前必须判断技术栈兼容性。Java 代码遵循 MyBatis + Spring Security 体系，禁止引入 JPA/Hibernate。**
...
| Skill | 路径 | 触发场景 |
|-------|------|---------|
| `senior-backend` | engineering-team/ | Node.js/Express + Java/Spring Boot API 设计、中间件、JWT、限流、微服务 |
...
| `performance-profiler` | Node.js/Java 性能瓶颈排查（CPU/内存/慢查询/GC/JVM/包体积） |
...
| 🔧 写 Spring Boot 代码 | `senior-backend` (claude-skills) | `backend-patterns` (ecc) |
| 🔧 写 MyBatis Mapper | `mysql-patterns` (ecc) | `sql-database-assistant` (claude-skills) |
...
| 🔧 设计数据库 Schema | `database-schema-designer` (claude-skills) | — |
| 🔧 数据库迁移（Flyway） | `database-migrations` (ecc) | `sql-database-assistant` (claude-skills) |
| 🔧 安全审查（含 Spring Security） | `security-review` (ecc) | `senior-security` (claude-skills) |
| 🔧 错误处理设计 | `error-handling` (ecc) | `senior-backend` (claude-skills) |
| 🔧 Redis 缓存设计 | `senior-backend` (claude-skills) | `mysql-patterns` (ecc) |
| 🔧 微服务配置（Gateway/Nacos/Sentinel） | `senior-architect` (claude-skills) | `brainstorming` (superpowers) |
| 🧪 写 Node.js 后端测试 | `api-test-suite-builder` (claude-skills) | `tdd-guide` (claude-skills) |
| 🧪 写 Java 单元测试 (JUnit 5/Mockito) | `test-driven-development` (superpowers) | `tdd-workflow` (ecc) |
| 🧪 TDD 红绿重构 | `test-driven-development` (superpowers) | `tdd-workflow` (ecc) |
| 🐛 Debug（任何层级/语言） | `systematic-debugging` (superpowers) | — |
...
| ⚡ Node.js 性能排查 | `performance-profiler` (claude-skills) | `mysql-patterns` (ecc) 慢查询 |
| ⚡ Java 性能排查 | `senior-backend` (claude-skills) | `performance-profiler` (claude-skills) |
...
- ❌ 在 Java 后端引入 JPA/Hibernate（项目已选 MyBatis + Mapper XML，与 Node.js 原生 SQL 风格一致）
- ❌ 对 `server-java/.env` 硬编码任何真实密钥/密码/手机号（受 gitignore 保护，仅通过 `${ENV_VAR}` 占位符引用）
- ❌ 在 Mapper XML 中使用 `SELECT *`（与 Node.js repository 层禁止 `SELECT *` 一致）
