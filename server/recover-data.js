...
try {
  execSync(
    `mysqldump -u root -p${DB_PASS} --single-transaction ${DB_NAME} > recover_backup_${Date.now()}.sql`,
...
try {
  ...
  const result = execSync(
    `mysqlbinlog --database=${DB_NAME} --force-read ${binlogArgs} | mysql -u root -p${DB_PASS} --force ${DB_NAME} 2>&1`,
...
try {
  ...
  const verify = es(
    `mysql -u root -p${DB_PASS} ${DB_NAME} -e "SELECT COUNT(*) as user_count FROM users; SELECT COUNT(*) as product_count FROM products; SELECT COUNT(*) as report_count FROM reports; SELECT COUNT(*) as order_count FROM orders; SELECT COUNT(*) as review_count FROM reviews;"`,
    { encoding: 'utf8', timeout: 10000, shell: 'cmd.exe' }
  );
  console.log(verify);
} catch (e) {
